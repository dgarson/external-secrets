...
func setAppRoleToken(
	ctx context.Context,
	vaultClient util.Client,
	authCtx *authContext,
) (bool, error) {
	appRole := authCtx.spec.Auth.AppRole
	if appRole != nil {
		err := requestTokenWithAppRoleRef(ctx, vaultClient, authCtx, appRole)
		if err != nil {
			return true, err
		}
		return true, nil
...
func requestTokenWithAppRoleRef(
	ctx context.Context,
	vaultClient util.Client,
	authCtx *authContext,
	...
	if appRole.RoleID != "" { // use roleId from CRD, if configured
		...
		roleID, err = resolvers.SecretKeyRef(ctx, authCtx.kube, authCtx.storeKind, authCtx.namespace, appRole.RoleRef)
		if err != nil {
			return err
		}
	...
	secretID, err := resolvers.SecretKeyRef(ctx, authCtx.kube, authCtx.storeKind, authCtx.namespace, &appRole.SecretRef)
	if err != nil {
		return err
	}
	...
	_, err = vaultClient.Auth().Login(ctx, appRoleClient)
